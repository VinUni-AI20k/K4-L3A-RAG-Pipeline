...
|---|---|---|---|---|
| Lê Duy Quân | Chưa cung cấp | Data Engineer | `feature/quan-data-ingestion` | Thu thập tài liệu pháp lý, crawl bài viết và chuẩn hóa Markdown. Phụ trách `src/task1_collect_legal_docs.py`, `src/task2_crawl_news.py`, `src/task3_convert_markdown.py` và `data/`. |
| Vũ Minh Hoàng | Chưa cung cấp | Retrieval Engineer | `feature/hoang-hybrid-retrieval` | Chunking, embedding, ChromaDB, semantic search, BM25 và RRF. Phụ trách `src/task4_chunking_indexing.py` đến `src/task7_reranking.py`. |
| Nguyễn Lê Phúc Thắng | Chưa cung cấp | RAG Integration & UI Engineer | `feature/thang-rag-generation-ui` | PageIndex fallback, hợp nhất retrieval pipeline, sinh câu trả lời có citation và giao diện chatbot. Phụ trách `src/task8_pageindex_vectorless.py`, `src/task9_retrieval_pipeline.py`, `src/task10_generation.py` và `app.py`. |
...
1. `feature/quan-data-ingestion`
2. `feature/hoang-hybrid-retrieval`
3. `feature/thang-rag-generation-ui`
