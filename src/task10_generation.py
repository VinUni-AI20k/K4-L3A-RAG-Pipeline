...
def call_llm(system_prompt: str, user_message: str) -> str:
    ...
    if LLM_PROVIDER == "anthropic":
        ...
        response = Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY")).messages.create(
            model=LLM_MODEL or "claude-3-5-haiku-latest", max_tokens=1500,
            system=system_prompt, messages=[{"role": "user", "content": user_message}],
        )
        return "".join(getattr(block, "text", "") for block in response.content)
    raise RuntimeError(f"Unsupported or unconfigured LLM_PROVIDER: {LLM_PROVIDER}")
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    try:
        ...
        answer = refusal
    return {"answer": answer or refusal, "sources": chunks, "retrieval_source": chunks[0]["retrieval_method"]}
