...
| Task 14 (bonus) — HyDE | LLM viết đoạn giả định theo văn phong descriptor, nối vào query; RRF gộp 3 danh sách (dense gốc, dense mở rộng, BM25 mở rộng) một lần; fallback vẫn theo cosine query gốc. Config D vs C: average 0.774 → 0.850, refusal 4 → 2; 4 test offline | `src/task14_hyde.py`, `tests/test_task14_hyde.py`, `results/config_D.json` | Done |
...
| UI highlight (bonus) | Citation trong answer → badge số có tooltip; nguồn được cite xếp trước, đánh số, viền màu; câu bằng chứng trong chunk bôi vàng theo trùng từ khoá/số; nguồn không cite hiển thị mờ; 6 test offline, ảnh chụp trong `docs/` | `src/ui_citations.py`, `app.py`, `tests/test_ui_citations.py`, `docs/ui_citation_highlight.png` | Done |
...
- Test hoặc query tôi đã dùng: `pytest -q` (52 passed); `python -m src.task9_retrieval_pipeline --calibrate` với 6 query in-domain + 6 out-of-domain; `python -m src.task11_evaluation` trên 20 câu golden cho cả A và B.
- Kết quả trước/sau nếu có: Config A → B: recall 0.788 → 0.848, faithfulness 0.683 → 0.700, precision 0.777 → 0.616. B → C (reranker): precision 0.616 → 0.868. C → D (HyDE): average 0.774 → 0.850, refusal 4 → 2. Sau khi sửa parser citation: refusal giả do format `[chunk_id: ...]` / thiếu tiền tố `news/` giảm về 0, còn lại 5–6 refusal đều do retrieval miss.
